...
_.realpathSafe = function(path) {
  // 如果路径存在，就能通过realpath获取真实路径
  // 如果路径不存在，就用_()做简单的normalize处理
...
_.read = function(path, convert) {
  ...
  if (_exists(path)) {
    ...
    if (convert || _.isTextFile(path)) {
      // 如果允许convert，且文件后缀认为文件是文本的话，那么解析内容成字符串
...
// 兼容处理[]、String、'String1, String2, ...'的配置写法
    ...
    processors = processors.filter(function(item, idx, arr) {
      ...
      // _.findIndex返回的是arr内第一个匹配名字的插件的下标
      // 如果idx与这个下标不相等则过滤
...
// 若type为多层级(ex: 'a.b')，获取fis.config中groups[defaultGroup]['modules']下一层配置项的名称
    ...
    processors.forEach(function(obj, index) {
      ...
      if (typeof processor === 'string') {
        key = type + '.' + processor;
        // 前缀(fis3,fis)，type和processor之间用'-'拼接加载模块
 ...
 * @function
 */
// path是mpath，this.subpath + (this.xLang || '') 
      ...
      keys.forEach(function(key) {
        ...
        if (typeof value === 'string') {
          value = value.replace(/\$(\d+|&)/g, function(_, k) {
            // 正则本身不支持'$0'，这里做了处理，
            // '$0'可以匹配match匹配到的整个字符串
            k = k === '&' ? 0 : k;
            return m[k] || '';
          });
        } else if (typeof(value) === 'function' &&
          // key不是以下4种时，执行value函数返回的值才作为value
          ~[
            'release',
            'url',
            'relative',
        ...
        if (value && value.__plugin && value.__pos) {
          ...
          // obj[key]是对象，value也是对象
          // 将value上的每一个属性复制到obj[key]
