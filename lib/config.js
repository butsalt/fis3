...
var Config = Object.derive(/** @lends fis.config.Config.prototype */{
  ...
  init: function() {
    ...
    // 存储pattern对应的配置
    this._matches = [];
    // 调用media会创建新的config，存储在数组里
    this._forks = [];
    // key是media，value是config
    this._groups = {};
    this._sortedMatches = null;
    // 在media方法中会将值改写成当前配置的media
    this._media = 'GLOBAL';
  ...
  match: function(pattern, properties, weight) {
    ...
    this._forks.forEach(function(forked) {
      // 子级config的getSortedMatches的缓存也要清除
  ...
  getSortedMatches: function() {
    // 缓存，如果当前配置或parent新增了match就清除缓存
    if (!this._sortedMatches) {
      var self = this;
      // 获取当前media和parent（如果存在）的配置
      this._sortedMatches = this.getMatches().map(function(match) {
        // negate为true时，表示要求查找所有符合pattern的文件
        // negate为false时，表示要求查找所有不符合pattern的文件
        var negate = false;
        // 如果pattern包含替换符，则需要先进行替换
        ...
        if (typeof pattern === 'string') {
          ...
          if (~pattern.indexOf('(') && ~pattern.indexOf('{')) {
            // 如果存在'('并且存在'{'，先要用minimatch做拆分
            // 'a{b,c}' -> ['ab', 'ac']
            // 然后再用fis.util.glob生成正则表达式
            // 因为在这种情况下，fis.util.glob无法正常解析'('成捕获
