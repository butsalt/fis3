...
var exports = module.exports = function(opt, callback) {
  ...
  hookLoaded || (fis.util.pipe('hook', function(processor, settings) {
    // 加载hook模块下名为components的processor
  ...
  var collect = function(file, type) {
    ...
    // 为真值时说明文件内容存在__RESOURCE_MAP__标识，需要替换
  ...
  // 在watch时会重复执行此方法，需要把上次执行时的事件监听解除
  ...
  fis.on('process:end', (onProcessEnd = function(file) {
    // 如果开启了同名引用策略，则对文件新增依赖
    if (file.useSameNameRequire) {
      if (file.isJsLike) {
        // 相同路径下，如果存在同名（不考虑后缀名），且产出的后缀名是css，就作为依赖
  ...
  // 所有通过fis.project.getSource检索出来的file都要进行
  ...
  // fis2写法，获取哪些文件需要被打包成一个文件的配置
  /**
      {
        'distSubPath': ['subPath1', subPath2]
      }
  **/
  //get pack config
  ...
  if (typeof conf === 'undefined') {
    ...
    if (fis.util.isFile(file)) {
      // fis-pack.json中的配置是全局的，所以直接设道fis.config上
      fis.config.set('pack', conf = fis.util.readJSON(file));
    }
  }

  if (typeof conf === 'undefined') {
    // fis3写法，直接在file的match上写packTo从而生成pack配置
    conf = buildPack(ret);
  }
  ...
  ['prepackager', 'packager', 'spriter', 'postpackager'].forEach(function(name) {
    ...
    if (packager[name]) {
      ...
      // packager的配置是当前media下的，设回当前media下的modules
