...
var exports = module.exports = function() {
  ...
  if (!flag) {
    ...
    if (!exports.paths.length) {
      // node_modules/fis3/lib -> node_modules/fis3 -> node_modules/fis3/node_modules
      exports.paths = [path.join(path.dirname(__dirname), 'node_modules')];
    }
  }

  var resolved = null;
  // paths最后一个path对应的应该是全局node_module的路径
  // 要根据这个路径逐层向上，每一级下的node_module都要考虑成为加载plugin的目录
  ...
  paths.every(function(dir) {
    names.every(function(name) {
      // loadAsDirectorySync会返回对应name的目录下的入口js的路径
...
function gatherAvailableNodePaths(paths) {
  paths = paths.concat();
  // start应是全局安装的fis3下的node_modules目录
  // 将会从此目录开始向上逐级解析，每级目录下的node_modules目录都是尝试加载plugin的目录
  var start = paths.pop();
  // 除start外的node_modules目录属于本地目录，直接使用
  ...
  for (var i = parts.length - 1; i >= 0; i--) {
    ...
    // 每一层文件夹下的node_modules都包含在查找范围内，越往上优先级越低
    dirs = dirs.concat(
      prefix + path.join(
