...
function getDomainsByPath(path) {
  // fis2的写法将domain放在'project.domain'
...
var File = Object.derive(function() {
  ...
  // 参数列表最后一个如果传的是对象，则用来为实例添加一些属性
  if (!_.isPlainObject(props)) {
    args.push(props);
    props = null;
  }

  // 根据文件路径（args用'/'拼接起来）解析基本信息
  ...
  // realpath需要是完整的真实路径，如果fullname已经是完整路径则直接使用，否则得用_.realpathSafe拉取
  ...
  if (realpath.indexOf(root) === 0) {
    // matches只能为本项目下的文件添加配置
    // 如果文件不在本项目下就不用查找配置了
    var len = root.length;
    var subpath;
    // 以工作目录作为开始的文件绝对路径，以'/'开始
    this.subpath = subpath = realpath.substring(len);
    // 以工作目录作为开始的文件所在目录的绝对路径，以'/'开始
    this.subdirname = self.dirname.substring(len);
    // 以工作目录作为开始的去掉后缀名的文件绝对路径，以'/'开始
    this.subpathNoExt = realpathNoExt.substring(len);

    // cache apply matches.
    // 获取当前media下的所有match（包括parent的）
    var matches = fis.media().getSortedMatches();
    var mcache = matchCache._owner === matches ? matchCache : (matchCache = {_owner: matches});
    // xLang是通过props传入的
    // isHtmlLike的文件中可以内嵌js，css等其他语言块，这些语言块最终被partial处理，逐个创建file
    // 每个file的路径同语言块所属文件的路径，但能通过props进行修改
    // xLang 以冒号开始，比如':inline-style'
  ...
  if (subpath) {
    // release为false的不参与release过程
    if (this.release === false && !this.url) {
      // 如果没有通过props配置url，这个文件不能进入resource map
      this.useMap = false;
      var self = this;
      Object.defineProperty(this, 'url', {
        enumerable: true,
        get: function() {
          // 这个文件没有url，如果要尝试读取的话就报错
          fis.log.error('unreleasable file [%s]', self.realpath);
        }
      });
    } else {
      // isMod的必须要加入到resource map，用于正确解释依赖关系
      this.useMap = this.isMod ? true : this.useMap;
      //release & url
      // '.css' -> new RegExp('.css') 无法用来识别
      // '.css' -> '\\.css' -> new RegExp('\\.css' + '$')
      var reg = new RegExp(_.escapeReg(ext) + '$|' + _.escapeReg(rExt) + '$', 'i');
      var release;
      if (this.release) {
        // release可能是match中用字符串声明的一个路径，基于产出的根目录
    ...
    // 获取当前配置中声明的项目有效扩展名列表
      ...
      for (var key in map) {
        if (map.hasOwnProperty(key)) {
          ...
          // releaseExt === ext，ext是编译后文件的ext
          // 并且当前文件的路径，去掉后缀名，拼接上'.' + 对应的源文件的ext所获得的这个路径对应的文件确实存在
