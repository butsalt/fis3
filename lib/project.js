...
exports.getSourceByPatterns = function(patterns, opts) {
  ...
  opts = _.assign({
    ...
    // 使得'*.js'这种没有'/'的pattern只要与文件名/文件夹名匹配就认为是满足匹配
    matchBase: true,
  ...
  patterns.forEach(function(pattern, index) {
    ...
    // 如果是'!'开头，则要从ret内把存在于mathes内的路径排除掉
    // 如果不是'!'开头，则要取ret和mathes的交集
...
exports.getTempRoot = function() {
  if (!TEMP_ROOT) {
    ...
    // __dirname是当前js文件所在的目录，'/../'可以定位到fis3目录
    tmp = tmp || __dirname + '/../';
    // fis3目录/.fis3-tmp
...
exports.lookup = function(path, file, silent) {
   ...
   * @memberOf fis
   */
  // hook会在准备release时被挂载，就是为了在emit事件时对特定事件进行处理从而扩展fis的功能
  // fis3内置fis3-hook-components，用于处理通过fis install安装的组件
  // fis install安装的组件被放置在/components文件夹下
  // 如果直接通过路径没有找到文件，此时会尝试在/components下查找
